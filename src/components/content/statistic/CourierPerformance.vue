<template>
    <div id="courierPerformance">
        <div id="courier-performance-map-svg"></div>
        <legend-bar></legend-bar>
        <div id="routine-detail">
            <div id="routine-detail-close" @click='closeRoutineDetail'>X</div>
        </div>
        <div id="worst-list">
            <div id="list-title">最糟糕的50名快递员</div>
            <div class="list-row-name">
                <div class="row-bt">详情</div>
                <div class="row-head">工号</div>
                <div class="row-time">超时总计</div>
                <div class="row-amount">包裹总计</div>
            </div>
            <div id="list-row-container">
                <div class='list-row' v-for='list in worstList' @click='clickList(list.key)'>
                    <div class="row-bt" @click='drawRoutineDetail(list.key)'>查看</div>
                    <div class="row-head">{{list.key}}</div>
                    <div class="row-time">{{list.value.penaltyTime}}</div>
                    <div class="row-amount">{{list.value.amountPackages}}</div>
                </div>
            </div>
        </div>
    </div>
...
function drawStatistic(_self) {
    ...
    var svg = d3.select("#courier-performance-map-svg").append("svg")
    ...
    function renderStaticMap(persistData) {
        ...
        var spotData = persistData[3]
        var shopData = persistData[2]
        ...
        drawCircles(g, spotData, projection, {radius:1, color:'#00BF5F'})
        drawCircles(g, shopData, projection, {radius:2, color:'#BF7B00'})
...
function drawCircles(svg, data, projection, style) {
    svg.append("g")
        .selectAll("circle")
        .data(data)
        .enter()
...
function drawRoutineDetail(routingId) {
    ...
    var lineArray = d3.range(timeGapsArray.length+1)
    timeGapGroup.selectAll('text.scheduleRoutineTimeGapText')
        .data(lineArray)
        .enter()
        .append('line')
